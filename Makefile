...
redis_features:
	set -e
	make go_deps
	cd tests_func/ && env -u COMPOSE_FILE FEATURE=$(FEATURE) REDIS_VERSION=$(REDIS_VERSION) go test -v -count=1 -timeout 20m  --tf.test=true --tf.debug=false --tf.clean=false --tf.stop=false --tf.database=redis

clean_redis_features:
	set -e
	cd tests_func/ && env -u COMPOSE_FILE FEATURE=$(FEATURE) REDIS_VERSION=$(REDIS_VERSION) go test -v -count=1  -timeout 5m --tf.test=false --tf.debug=false --tf.clean=true --tf.stop=true --tf.database=redis
