...
2. Using a config file
   ...
   `--config /path` flag can be used to specify the path where the config file is located. Alternatively, you can set the `WALG_CONFIGFILE` environment variable to specify the config file path, so you don't need to pass the flag every time.
