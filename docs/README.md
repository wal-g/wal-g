...
## Dirty hands mode (danger zone)
"Dirty hands" command series allows interacting with the configured storage. Be aware that this command can do potentially harmful operations and make sure that you know what you're doing.
...
``wal-g dh folder-list`` get listing with all objects in the configured storage.

``wal-g dh folder-list some_folder/some_subfolder`` get listing with all objects in the provided storage path.
