...
Summaries are written per timeline, so a range spanning a promotion needs the ancestor timelines' summaries as well. WAL-G reads `$PGDATA/pg_wal/<timeline>.history` to find the switchpoints and requires complete coverage of each timeline's slice of the range, the same way PostgreSQL's own incremental backup does.
