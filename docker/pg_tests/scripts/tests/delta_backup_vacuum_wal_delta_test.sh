...
psql -c "DROP DATABASE IF EXISTS test" postgres
psql -c "CREATE DATABASE test" postgres
psql -c "CREATE TABLE heap AS SELECT a FROM generate_series(1,4000) AS a;" -d test
psql -c 'vacuum heap;' -d test
wal-g --config=${TMP_CONFIG} backup-push ${PGDATA}
psql -c "DELETE from heap WHERE a > 2000;" -d test
psql -c "INSERT INTO heap SELECT a FROM generate_series(4000,5000) as a;" -d test
psql -c 'vacuum heap;' -d test
