...
export WALE_S3_PREFIX=s3://deleteretainfindfullbucket
...
wal-g backup-list | tail -n 4 > /tmp/list_tail_before_delete

wal-g delete retain FIND_FULL 3 --confirm

wal-g backup-list
lines_after_delete=`wal-g backup-list | wc -l`
wal-g backup-list | tail -n 4 > /tmp/list_tail_after_delete
