...
# init config
CONFIG_FILE="/tmp/configs/wal_restore_remote_test_config.json"
...
timeout 30 wal-g --config=${TMP_CONFIG} wal-restore ${PGDATA_ALPHA} ${PGDATA_BETA} --remote --private-key-path-ssh=/private_key.id_rsa
sleep 10
