...
psql -p 7000 -d test -c "INSERT INTO co select i, i FROM generate_series(1,1000000)i;"
psql -p 7000 -d test -c "INSERT INTO ao select i, i FROM generate_series(1,1000000)i;"
...
psql -p 7000 -d test -c "INSERT INTO ao select i, i FROM generate_series(1000000,2000000)i;"
psql -p 7000 -d test -c "INSERT INTO co select i, i FROM generate_series(1000000,2000000)i;"
psql -p 7000 -d test -c "BEGIN; INSERT INTO ao select i, i FROM generate_series(10000000,20000000)i; SELECT pg_sleep(20); ROLLBACK;" &
psql -p 7000 -d test -c "BEGIN; INSERT INTO co select i, i FROM generate_series(10000000,20000000)i; SELECT pg_sleep(20); ROLLBACK;" &
sleep 5
...
psql -p 7000 -d test -c "DELETE FROM co WHERE a>0;"
psql -p 7000 -d test -c "DELETE FROM ao WHERE a>0;"
psql -p 7000 -d test -c "VACUUM FULL;"
psql -p 7000 -d test -c "INSERT INTO ao select i, i FROM generate_series(10000000,20000000)i;"
psql -p 7000 -d test -c "INSERT INTO co select i, i FROM generate_series(10000000,20000000)i;"
run_backup_logged ${TMP_CONFIG} ${PGDATA}
var=$(wal-g --config=${TMP_CONFIG} st ls segments_005/seg0/basebackups_005/aosegments/ | grep aoseg | wc -l)
if [ "$var" -ne 12 ] ; then
...
start_cluster
psql -p 7000 -d test -c "SELECT COUNT(*) FROM ao;"

psql -p 7000 -d test -c "SELECT COUNT(*) FROM ao;" | grep 10000001 && EXIT_STATUS=$? || EXIT_STATUS=$?
if [ "$EXIT_STATUS" -ne 0 ] ; then
    echo "Error: Failed to read from ao table after restore"
    exit 1
fi

psql -p 7000 -d test -c "SELECT COUNT(*) FROM co;" | grep 10000001 && EXIT_STATUS=$? || EXIT_STATUS=$?
if [ "$EXIT_STATUS" -ne 0 ] ; then
