...
# Plain GTIDs are covered by pitr_binlog_server_test.sh in this directory.
if [ "$#" -eq 0 ]; then
...
case "$WALG_MYSQL_TEST_SERVER_VERSION" in
    8.4.*|9.7.*) ;;
    5.7.*|8.0.*)
        echo "Skipping tagged GTID test: MySQL $WALG_MYSQL_TEST_SERVER_VERSION does not support tagged GTIDs"
        exit 0
        ;;
    *)
