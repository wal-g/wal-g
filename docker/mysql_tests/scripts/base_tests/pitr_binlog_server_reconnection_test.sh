...
log "=== Checking replicated data ==="
mysql -e "SELECT id, ts FROM sbtest.pitr ORDER BY ts, id" | while read line; do
    log "$line"
done
log "Total rows: $(mysql -N -e 'SELECT COUNT(*) FROM sbtest.pitr')"
