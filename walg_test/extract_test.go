...
func TestNoFilesProvided(t *testing.T) {
	...
	assert.Errorf(t, err, "extract: Did not catch no files provided error")
}
...
func testLzopRoundTrip(t *testing.T, stride, nBytes int) {
	...
	go func() {
		testtools.CreateTar(memberW, &io.LimitedReader{
			R: bytes.NewBuffer(b),
			N: int64(len(b)),
		})
		memberW.Close()
	}()
	comReader := &testtools.LzopReader{Uncompressed: tarR}
	lzopTarReader := bytes.NewBufferString(testtools.LzopPrefix)
...
func TestLzopUncompressableBytes(t *testing.T) {
	testLzopRoundTrip(t, testtools.LzopBlockSize*2, testtools.LzopBlockSize*2)
}
...
func setupRand(stride, nBytes int) *BufferReaderMaker {
	...
	b := &BufferReaderMaker{bytes.NewBufferString(testtools.LzopPrefix), "/usr/local", "lzo"}

	pr, pw := io.Pipe()

	go func() {
		testtools.CreateTar(pw, lr)
		defer pw.Close()
	}()

	comReader := testtools.LzopReader{Uncompressed: pr}
