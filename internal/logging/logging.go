...
// SetupLogger configures slog with a level and format
...
func FatalfOnError(format string, err error) {
	if err != nil {
		Fatalf(format, err)
	}
}
...
func FatalError(err error, args ...any) {
	Fatalf(fmt.Sprintf(GetErrorFormatter(), err), args)
}
...
func PrintError(err error) {
	slog.Error(fmt.Sprintf(GetErrorFormatter(), err))
}
