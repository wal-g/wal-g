...
func addMySQLHistoriesToPlan(
	...
	if hasBinlogSentinel {
		plan.AddInline(BinlogSentinelPath, binlogSentinelSnapshot, copy.RecoveryMetadataPhase, true)
	}
	return nil
}
...
func addMySQLBackupChain(
	...
	if prefix != "" {
		...
		target := path.Join(strings.TrimSuffix(utility.BaseBackupPath, "/"),
			internal.SentinelNameFromBackup(prefix+name))
		plan.AddInline(target, data, copy.BackupCommitPhase, false)
	}
	target := path.Join(strings.TrimSuffix(utility.BaseBackupPath, "/"),
		internal.SentinelNameFromBackup(prefix+name))
	if err := plan.SetOrder(target, copy.BackupCommitPhase, depth); err != nil {
		return StreamSentinelDto{}, 0, err
	}
	depths[name] = depth
	state[name] = 2
