...
// in xtrabackup_info / mariadb_backup_info. GTID presence depends on whether the source
// has GTID enabled, not on which tool wrote the file -- xtrabackup records one too when
// MySQL's GTID_MODE is on.
// Formats:
//
//	"filename 'mysql-bin.000002', position '607', GTID of the last change '0-1-7'" (with GTID)
//	"filename 'mysql-bin.000003', position '154'" (no GTID recorded)
