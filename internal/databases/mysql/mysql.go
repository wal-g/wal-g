...
func provideLogs(ctx context.Context, folder storage.Folder, dstDir string, startTS, endTS time.Time,
	...
	defer func() {
		...
		tracelog.InfoLogger.Printf("[Session %d] provideLogs: Closing provider", sessionID)
		p.Close()
	...
	for _, logFile := range logsToFetch {
		...
		if _, err := os.Stat(binlogPath); err == nil {
			tracelog.DebugLogger.Printf("[Session %d] provideLogs: Removing existing file %s", sessionID, binlogPath)
		...
		if err != nil {
			if errors.Is(err, context.Canceled) || errors.Is(ctx.Err(), context.Canceled) {
				tracelog.InfoLogger.Printf("[Session %d] provideLogs: Context cancelled while adding %s to provider",
					sessionID, binlogName)
				os.Remove(binlogPath)
				return
			}
			if errors.Is(err, storage.ErrProviderClosed) {
				tracelog.InfoLogger.Printf("[Session %d] provideLogs: Provider closed while adding %s",
					sessionID, binlogName)
				os.Remove(binlogPath)
				return
			}
