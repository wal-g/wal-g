...
Each **segment** records what its uploaders initially pushed as ``UploadedSharedSize`` in its own ``ao_files_metadata.json`` and ``pax_files_metadata.json`` — a deduplicated file never reaches the uploader and is not counted. This value remains immutable and can stop matching current ownership after an older backup is deleted. The **coordinator** initially sums these values into the cluster-level pair as ``SharedSize``.

AO and PAX stay in separate objects all the way up — separate deduplication age limits, separate cleanup passes.

When a backup is deleted, ownership is recomputed for the first surviving backup after each deleted block. Segment history is linear: references to one physical ``StoragePath`` form a continuous interval, and a logical file that disappears and later returns gets a new storage path. Therefore a backup owns the files it references that its immediately preceding surviving backup does not. All later backups keep the same predecessor and ownership. Reassignment reads a narrow ``StoragePath`` plus ``EOF``/``Size`` view of files metadata only for the affected segment backup and that predecessor. The separate physical cleanup pass scans all surviving metadata to build the set of storage objects that must be retained.

The reassigned size is the sum of those owned files: ``EOF`` from AO metadata and ``Size`` from PAX metadata. After all segment cleanup finishes, the coordinator sums the reassigned values and replaces only the small cluster-level ``SharedSize`` objects for the corresponding affected backups. Segment files metadata is never rewritten.
