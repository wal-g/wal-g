...
func TestGreenplumPlainCopyDoesNotResolveRestorePointTimeline(t *testing.T) {
	postgres.SetWalSize(64)
	from := testtools.MakeDefaultInMemoryStorageFolder()
	to := testtools.MakeDefaultInMemoryStorageFolder()
	now := time.Now().UTC()
	...
	putDTO(t, from, path.Join(utility.BaseBackupPath, greenplum.RestorePointMetadataFileName(restorePoint)),
		greenplum.RestorePointMetadata{
			...
			TimelineBySegment: map[int]uint32{0: 2},
		})
	for i := 1; i <= 3; i++ {
		walName := postgres.WalSegmentNo(i).GetFilename(1)
	...
	endpointWal1 := path.Join(segmentRoot, utility.WalPath, postgres.WalSegmentNo(4).GetFilename(1))
	endpointWal2 := path.Join(segmentRoot, utility.WalPath, postgres.WalSegmentNo(4).GetFilename(2))
	require.NoError(t, from.PutObject(t.Context(), endpointWal1, bytes.NewBufferString("endpoint-wal")))
	require.NoError(t, from.PutObject(t.Context(), endpointWal2, bytes.NewBufferString("ambiguous-endpoint-wal")))
	...
	lastWal := path.Join(segmentRoot, utility.WalPath, postgres.WalSegmentNo(3).GetFilename(1))
	require.True(t, targets[lastWal])
	require.False(t, targets[endpointWal1])
	require.False(t, targets[endpointWal2])
	require.True(t, targets[path.Join(
		utility.BaseBackupPath, greenplum.RestorePointMetadataFileName(restorePoint))])
}
