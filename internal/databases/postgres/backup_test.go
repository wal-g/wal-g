...
func TestGetFilesToUnwrap_SimpleFile(t *testing.T) {
	...
	files, _ := backup.GetFilesToUnwrap(t.Context(), "")
...
func TestCheckExistenceWhenBackupExists(t *testing.T) {
	...
	exists, err := backup.CheckExistence(t.Context())
...
func TestGetTarNames(t *testing.T) {
	...
	require.NoError(t, err)
	tarNames, err := backup.GetTarNames(t.Context())
	assert.NoError(t, err)
...
func TestIsPgControlRequired(t *testing.T) {
	...
	backup, err := postgres.NewBackup(folder.GetSubFolder(utility.BaseBackupPath), "base_456")
	require.NoError(t, err)
	_, err = backup.GetSentinel(t.Context())
	assert.NoError(t, err)
...
func TestFetchSentinel(t *testing.T) {
	...
	_ = folder.PutObject(t.Context(), "base_789454598_backup_stop_sentinel.json", bytes.NewReader(expectedSentinelJson))
	backup, err := postgres.NewBackup(folder, "base_789454598")
	require.NoError(t, err)

	actualSentinel, err := backup.GetSentinel(t.Context())
...
func TestGetLatestBackupName(t *testing.T) {
	...
	for _, nameBackupPrefix := range backupNames {
		...
		_ = folder.PutObject(t.Context(), nameBackup, &bytes.Buffer{})

		latestBackup, err := internal.GetLatestBackup(t.Context(), folder)
		assert.NoError(t, err)
...
func TestGetLatestBackupNameNoBackupsInFolder(t *testing.T) {
	...
	backup, err := internal.GetLatestBackup(t.Context(), baseBackupFolder)
...
func TestGetLastBackupNameWithGarbage(t *testing.T) {
	...
	latestBackup, err := internal.GetLatestBackup(t.Context(), subFolder)
