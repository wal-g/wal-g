...
func (bb *StreamingBaseBackup) Upload(uploader internal.Uploader, bundleFiles internal.BundleFiles) (err error) {
	...
	for {
		...
		compressedFile := internal.CompressAndEncrypt(tbsTar, bb.uploader.Compression(), internal.ConfigureCrypter(), bb.uploader.CompressedSizePtr())
	...
	if len(bb.streamer.Tee) > 0 {
		...
		teeCompressedFile := internal.CompressAndEncrypt(teeTar, bb.uploader.Compression(), internal.ConfigureCrypter(), bb.uploader.CompressedSizePtr()) //nolint:lll
