...
func HandleBackupExport(externalConfig string, exportPrefixes map[string]string) {
	...
	slog.Info(fmt.Sprintf("export finished"))
}
...
func exportSingleDatabaseBackup(ctx context.Context, folder storage.Folder, backupName string,
	...
	slog.Info(fmt.Sprintf("starting exporting backup files for %v from %v", dbname, blobs))
	for i := 0; i < len(urls); i++ {
		...
		slog.Info(fmt.Sprintf("backup file %v exported", backupFileName))
	}
	slog.Info(fmt.Sprintf("all backup files for %v are exported successfully", dbname))
	return nil
}
