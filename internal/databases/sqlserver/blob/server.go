...
func (bs *Server) HandleBlockPut(w http.ResponseWriter, req *http.Request) {
	...
	err = folder.PutObject(filename, internal.CompressAndEncrypt(req.Body, bs.compressor, bs.crypter, nil))
	<-bs.uploadSem
	req.Body.Close()
	if err != nil {
		bs.returnError(w, req, err)
...
func (bs *Server) blobPut(r io.Reader, idx *Index) ([]string, error) {
	...
	for i, last := 0, false; !last; i++ {
		...
		encryptedReader := internal.CompressAndEncrypt(bytes.NewReader(buf[:n]), bs.compressor, bs.crypter, nil)
