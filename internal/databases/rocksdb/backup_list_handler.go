...
func writeBackupListDetails(backupDetails []BackupInfo, output io.Writer) error {
	...
	_, err := fmt.Fprintln(writer, "BackupName\tbackup_size\tbackuptime") //nolint:lll
	if err != nil {
		return err
	}
	for i, count := 0, len(backupDetails); i < count; i++ {
		...
		_, err = fmt.Fprintf(writer, "%v\t%v\t%v\n",
			b.BackupName, b.Size, time.Unix(int64(b.Timestamp), 0).Format(time.RFC850)) //nolint:lll
		if err != nil {
			return err
		}
	}
...
func writePrettyBackupListDetails(backupDetails []BackupInfo, output io.Writer) {
	...
	defer writer.Render()
	writer.AppendHeader(table.Row{"#", "BackupName", "Backup size", "BackupDate"}) //nolint:lll
	for idx := range backupDetails {
		b := &backupDetails[idx]
		writer.AppendRow(table.Row{idx + 1, b.BackupName, b.Size, time.Unix(int64(b.Timestamp), 0).Format(time.RFC850)}) //nolint:lll
	}
}
