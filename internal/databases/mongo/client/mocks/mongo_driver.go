...
// ChangeOplogLastTimestamp provides a mock function with given fields: ctx, ts
func (_m *MongoDriver) ChangeOplogLastTimestamp(ctx context.Context, ts primitive.Timestamp) error {
	ret := _m.Called(ctx, ts)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, primitive.Timestamp) error); ok {
		r0 = rf(ctx, ts)
	} else {
		r0 = ret.Error(0)
	}
...
func (_m *MongoDriver) LastOplogTS(ctx context.Context) (models.Timestamp, error) {
	...
	if len(ret) == 1 {
		rf, ok := ret.Get(0).(func(context.Context) (models.Timestamp, error))
		if ok {
			return rf(ctx)
		}
	}
