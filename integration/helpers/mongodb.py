...
WALG_CONF_PATH = '/config/wal-g.conf'
WALG_DEFAULT_ARGS = ''
...
def delete_backup_entry(instance, backup_entry, cli_path=None, conf_path=None):
    ...
    if conf_path is None:
        conf_path = WALG_CONF_PATH
    _, output = docker.exec_run(
        instance, '{cli_path} -c {conf_path} -p {backup_entry} delete'.format(
            cli_path=cli_path, conf_path=conf_path, backup_entry=backup_entry))
    return output


def purge_backups(instance, cli_path=None, conf_path=None):
    """
    ...
    if conf_path is None:
        conf_path = WALG_CONF_PATH
    _, output = docker.exec_run(instance, '{cli_path} delete'.format(
        cli_path=cli_path))
    print(conf_path)
    return output
...
def restore_backup_entry(instance, backup_entry, cli_path=None, conf_path=None):
    ...
    if conf_path is None:
        conf_path = WALG_CONF_PATH
    _, output = docker.exec_run(
        instance, '{cli_path} -c {conf_path} -p {backup_entry} restore'.format(
            cli_path=cli_path, conf_path=conf_path, backup_entry=backup_entry))
    return output
